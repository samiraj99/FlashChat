...
class ChatViewController: UIViewController {
    ...
    func loadMessages() {
        db.collection(K.FStore.collectionName).order(by: K.FStore.dateField).addSnapshotListener { (queysnapshot, error) in
            if let e = error {
                print("Error",e)
            }else{
                if let snapshotDocument = queysnapshot?.documents{
                    self.messages = []
...
extension ChatViewController : UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let currentUserEmail = Auth.auth().currentUser?.email {
            ...
            if currentUserEmail == messages[indexPath.row].sender {
